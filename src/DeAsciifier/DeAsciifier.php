...
class DeAsciifier {
    ...
    private $Veri = [];
    private $turkishToggleAccentTable = [];
    private $turkishContextSize = 10;

    public function __construct() {
        $this->turkishToggleAccentTable = ["c" => "ç", "C" => "Ç", "g" => "ğ", "G" => "Ğ", "o" => "ö", "O" => "Ö", "u" => "ü", "U" => "Ü", "i" => "ı", "I" => "İ", "s" => "ş", "S" => "ş", "ç" => "C", "Ç" => "C", "ğ" => "g", "Ğ" => "G", "ö" => "o", "Ö" => "O", "ü" => "u", "Ü" => "U", "ı" => "i", "İ" => "I", "ş" => "s", "Ş" => "s" ];
        $this->turkishAsciifyTable = ["ğ" => "g", "ü" => "u", "ş" => "s", "ı" => "i", "ö" => "o", "ç" => "c"];
        $this->turkishDowncaseAsciiTable = ["a" => "a", "b" => "b", "c" => "c", "d" => "d", "e" => "e", "f" => "f", "g" => "g", "h" => "h", "j" => "j","k" => "k","l" => "l","m" => "m","n" => "n","o" => "o","p" => "p","r" => "r","s" => "s","t" => "t","u" => "u","v" => "v", "y" => "y","z" => "z", "i" => "i", "ç" => "c", "Ç" => "c", "ğ" => "g", "Ğ" => "g", "ö" => "o", "Ö" => "o", "ı" => "i", "İ" => "i", "ş" => "s", "Ş" => "s", "ü" => "u", "Ü" => "u"];
        $this->turkishUpcaseTable = ["a" => "a", "b" => "b", "c" => "c", "d" => "d", "e" => "e", "f" => "f", "g" => "g", "h" => "h", "j" => "j","k" => "k","l" => "l","m" => "m","n" => "n","o" => "o","p" => "p","r" => "r","s" => "s","t" => "t","u" => "u","v" => "v", "y" => "y","z" => "z", "i" => "i", "ç" => "C", "Ç" => "C", "ğ" => "G", "Ğ" => "G", "ö" => "O", "Ö" => "O", "ı" => "I", "İ" => "i", "I" => "I", "ş" => "S", "Ş" => "S", "ü" => "U", "Ü" => "U"];
        $this->Veri = json_decode('{"c":
            ...
            "s": {"kut Xe": 1, "a nurX": 2, "lIXiyi": -3, "Xleriz": -4, "raXama": 5,
            ...
            "u": {"Xctugu": -1, " ay sX": 2, "fXtur ": 3, "Xysuz ": -4,
                   ...
                   "gX": 2407, "X": -2408},
            "i": {"n kXsan": -1, " nin Xn": -2, "tIyor X": -3, " armanX": -4,
    ...
    public function setCharAt ($str, $yer, $x) {
        return substr($str, 0, $yer) . $x . substr($str, $yer + 1);
    }

    public function turkishToggleAccent ($Yer) {
        $Parca = mb_str_split($this->Turkce);
        $Harf = $Parca[$Yer];

        if (array_key_exists($Harf, $this->turkishToggleAccentTable)) {
            $Parca[$Yer] = $this->turkishToggleAccentTable[$Harf];
            $this->Turkce = implode($Parca);
        }
    }

    public function turkishGetContext ($Yer, $Size = 10) {
        $s = str_repeat(" ", (($Size * 2) + 1));
        $s = substr($s, 0, $Size) . "X" . substr($s, $Size + 1);

        $Turkce = mb_str_split($this->Turkce);
        $Bosluk = False;
        $Harf = $Turkce[$Yer];
        ...
        while ($i < strlen($s) && !$Bosluk && $Index < strlen($this->Turkce)) {

            if (empty($Turkce[$Index]) or empty($this->turkishDowncaseAsciiTable[$Turkce[$Index]])) {
                if (!$Bosluk) {
                    $i++;
                    $Bosluk = True;
                }
                ...
                $s = substr($s, 0, $i) . $Harf . substr($s, $i + 1);
                $Bosluk = False;
                $i++;
            }
        ...
        while ($i >= 0 && $Index >= 0) {
            $Harf = $Turkce[$Index];
            if (empty($this->turkishUpcaseTable[$Harf])) {
                if (!$Bosluk) {
                    $i--;
                    $Bosluk = True;
                }
            } else {
                $s = substr($s, 0, $i) . $this->turkishUpcaseTable[$Harf] . substr($s, $i+1);
                $Bosluk = False;
                $i--;
            }
    ...
    public function turkishMatchPattern($Liste, $Yer) {
        $Start = 0;
        $End = 0;
         
        $Rank = count($Liste) * 2;

        $Str = $this->turkishGetContext($Yer);
        $Uzunluk = strlen($Str);

        while ($Start <= $this->turkishContextSize) {
            $End = $this->turkishContextSize + 1;
            while ($End <= $Uzunluk) {
                $Sub = substr($Str, $Start, $End - $Start);
                if (array_key_exists($Sub, $Liste) && abs($Liste[$Sub]) < abs($Rank)) {
                    $Rank = $Liste[$Sub];
                }
                $End++;
            }
    ...
    public function turkishNeedCorrection ($Yer) {
        $Parca = mb_str_split($this->Turkce);
        $Harf = $Parca[$Yer];

        $Tr = !empty($this->turkishAsciifyTable[$Harf]) ? $this->turkishAsciifyTable[$Harf] : $Harf;
        if (!empty($this->Veri[$Harf])) {
            $M = $this->turkishMatchPattern($this->Veri[$Harf], $Yer);
        } else {
            $M = false;
        }
    ...
    public function turkishCorrectRegion ($Baslangic, $Son) {
        for ($i = $Baslangic; $i < $Son; $i++) {
            if ($this->turkishNeedCorrection($i)) {
                $this->turkishToggleAccent($i);
            }
        }
    ...
    public function deasciifyRange ($Yazi, $Baslangic, $Son) {
        $this->Yazi = $Yazi;
        $this->Turkce = $Yazi;

        $this->turkishCorrectRegion($Baslangic, $Son);

        return $this->Turkce;
    }

    public function DeAsciify ($Yazi) {
        if ($Yazi == null || strlen($Yazi) == 0)
        {
            return $Yazi;
        }
        return $this->deasciifyRange($Yazi, 0, strlen($Yazi));
    }
}